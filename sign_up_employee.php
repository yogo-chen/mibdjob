...
<body class="w3-light-grey">
	...
	<div class="w3-main w3-animate-right" style="margin-left: 275px">
		...
		<div class="w3-container w3-content" style="padding-top: 96px">
			<div class="w3-container w3-section w3-card-2 w3-white">
				<h3 class="w3-center">Employee Sign Up</h3>
				<hr>
				<form method="post" action="./action_sign_up_employee.php" onsubmit="return validate()">
					<?php 
					...
					<div class="w3-row">
						...
						<select name="company_id" class="w3-col s8 m8 l8 w3-input" style="padding-top: 17px;" id="company_select" onchange="checkCompany()">
							<option value="undefined" class="w3-text-grey">select your company</option>
							<option value="new" class="w3-text-teal">Register a new company</option>
							<?php
								include "./server.php";
								$sql = ("SELECT id_perusahaan, nama FROM perusahaan ORDER BY nama ASC");
								$sth = $dbh->prepare($sql);
								$sth->execute();
								if($res = $sth->fetchAll()){
									foreach ($res as $row) {
							?>
							<option value=<?php echo "\"".$row["id_perusahaan"]."\""; ?> class="w3-text-black"><?php echo $row["nama"]; ?></option>
							<?php
									}
								}
							?>
					...
					<div class="w3-row w3-hide" id="company_name_container">
						<p class="w3-col s3 m2 l2 w3-right-align">
							<i class="fa fa-plus w3-margin-right"></i>
						</p>
						<p class="w3-col s1 m2 l2">
							<span class="w3-hide-small"><b>Create new</b></span>
						</p>
						<input type="text" placeholder="your company name" name="company_name" class="w3-col s8 m8 l8 w3-input" style="padding-top: 14px;" id="company_name">
					</div>
					<div class="w3-row">
						<p class="w3-col s3 m2 l2 w3-right-align">
						...
						<input type="text" placeholder="your division" name="division" class="w3-col s8 m8 l8 w3-input" style="padding-top: 14px;" required>
					</div>
	...
	function checkCompany(){
		...
		if(company_select.options[company_select.selectedIndex].value === "undefined"){
			company_select.classList.add("w3-border-red");
			company_select.classList.remove("w3-text-teal");
			company_name_container.classList.add("w3-hide");
			company_name.removeAttribute("required");
			return false;
		} else if(company_select.options[company_select.selectedIndex].value === "new"){
			company_select.classList.remove("w3-border-red");
			company_select.classList.add("w3-text-teal");
			company_name_container.classList.remove("w3-hide");
	...
	function validate(){
		return checkPassword() && checkCompany();
	}
	</script>
</body>
</html>