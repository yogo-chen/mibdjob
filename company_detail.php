			...
			<div class="w3-container w3-section w3-padding-hor-16 w3-card-2 w3-white">
				<?php
					if(isset($_GET["id"]) && ($id = $_GET["id"]) !== ""){
						$sql = ("SELECT id_perusahaan, nama FROM perusahaan WHERE id_perusahaan = :id_perusahaan");
						$params = array(":id_perusahaan" => $id);
						$sth = $dbh->prepare($sql);
						$sth->execute($params);

						if(!$res = $sth->fetch()){
				?>
				<p class="w3-large w3-text-teal">Company not found.</p>
				<?php
						}else{
				?>
				<h3 class="w3-center"><?php echo $res["nama"] ?></h3>
				<?php
							$sql = ("SELECT l.id_lowongan id_lowongan, l.id_perusahaan id_perusahaan, l.gaji gaji, l.posisi posisi, p.nama nama_perusahaan FROM lowongan l INNER JOIN perusahaan p ON l.id_perusahaan = p.id_perusahaan WHERE p.id_perusahaan = :id_perusahaan AND l.tanggal_tutup > CURRENT_DATE ORDER BY l.tanggal_tutup ASC");
							$params = array(":id_perusahaan" => $id);
							$sth = $dbh->prepare($sql);
							$sth->execute($params);
...
?>
				<hr>
				<p class="w3-large w3-text-teal">No open job at the moment.</p>
<?php
							}
						}
				?>
